...
class DataValidation:
    ...
    def detect_dataset_drift(self, reference_df: DataFrame, current_df: DataFrame) -> bool:
        ...
        try:
            ...
            # 1. Drop ID column
            # =========================
            if "case_id" in reference_df.columns:
                reference_df = reference_df.drop(columns=["case_id"])
            if "case_id" in current_df.columns:
                current_df = current_df.drop(columns=["case_id"])

            # =========================
            # 2. Fix Data Types
            # =========================
            # Convert object columns to category (better for drift detection)
            categorical_cols = reference_df.select_dtypes(include="object").columns

            for col in categorical_cols:
                reference_df[col] = reference_df[col].astype("category")
